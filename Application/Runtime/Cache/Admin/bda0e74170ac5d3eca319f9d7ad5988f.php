    ...
    <?php if(is_array($data)): $k = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($k % 2 );++$k;?><tr>
        ...
        <td>
          <div class="hidden-sm hidden-xs btn-group">
            <a class="btn btn-xs btn-info" href="<?php echo U('recover','id='.$item['id']);?>" title="商品还原">
              <i class="ace-icon fa fa-refresh"></i>
            </a>
            <button class="btn btn-xs btn-warning" title="查看详情">
              <i class="ace-icon fa fa-envelope"></i>
            </button>
            <button class="btn btn-xs btn-danger" onclick="deleteItem('<?php echo ($item["id"]); ?>')" title="彻底删除">
